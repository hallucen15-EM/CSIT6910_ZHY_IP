"""Agent loop for token-audit evaluation.

Streams the model's responses, executes tool calls against the MCP server,
and handles grading.
"""

import asyncio
import json
import logging
from datetime import datetime, timezone
from pathlib import Path
from typing import Any, Optional

from client import MCPClient
from transcript_writer import TranscriptWriter

logger = logging.getLogger(__name__)


async def stream_agentic_response(
    mcp_client: MCPClient,
    model: str,
    max_tokens: int,
    problem_id: str,
    transcript_path: Optional[str] = None,
    thinking_budget: int = 10000,
):
    """Stream model responses and execute tools against the MCP server."""

    transcript_writer = None
    if transcript_path is None:
        repo_root = Path(__file__).parent.parent
        transcripts_dir = repo_root / ".transcripts"
        transcripts_dir.mkdir(exist_ok=True)
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        transcript_filename = f"{problem_id}_{timestamp}.json"
        transcript_path = str(transcripts_dir / transcript_filename)

    transcript_writer = TranscriptWriter(transcript_path, problem_id, "local")

    def yield_event(event_type: str, **kwargs):
        event_data = f"data: {json.dumps({'type': event_type, **kwargs})}\n\n"
        if transcript_writer:
            transcript_writer.write_event(event_data)
        return event_data

    try:
        await mcp_client.connect_to_server()
        prompt = await mcp_client.get_prompt()
        tools = await mcp_client.get_tools()

        yield yield_event("start", tools=tools)

    except Exception as e:
        print(f"Error setting up server: {e}")
        yield yield_event("error", content="Error setting up server: " + str(e))
        return

    assert isinstance(prompt, list)

    content = []
    for block in prompt:
        if hasattr(block, "text"):
            content.append({"type": "text", "text": block.text})

    messages = [{"role": "user", "content": content if len(content) > 1 else content[0]["text"] if content else ""}]
    yield yield_event("message", message=messages[0])

    grade_called = False
    report_submitted = False
    total_prompt_tokens = 0
    total_completion_tokens = 0
    usage_from_api = False

    try:
        while True:
            current_text = ""
            tool_calls = []

            for retry_attempt in range(10):
                try:
                    current_text = ""
                    collected_tool_calls = {}
                    finish_reason = None

                    api_params = {
                        "model": model,
                        "max_completion_tokens": max_tokens,
                        "messages": messages,
                        "tools": tools if tools else None,
                        "stream": True,
                    }

                    if model.startswith("glm-"):
                        api_params.pop("max_completion_tokens", None)
                        api_params["max_tokens"] = max_tokens
                        api_params["stream_options"] = {"include_usage": True}

                    response = await mcp_client.client.chat.completions.create(**api_params)

                    async for chunk in response:
                        if chunk.choices and chunk.choices[0].finish_reason:
                            finish_reason = chunk.choices[0].finish_reason

                        delta = chunk.choices[0].delta if chunk.choices else None
                        if not delta:
                            continue

                        if delta.content:
                            current_text += delta.content
                            yield yield_event("text", content=delta.content)

                        if delta.tool_calls:
                            for tool_call_delta in delta.tool_calls:
                                idx = tool_call_delta.index
                                if idx not in collected_tool_calls:
                                    collected_tool_calls[idx] = {"id": "", "name": "", "arguments": ""}
                                if tool_call_delta.id:
                                    collected_tool_calls[idx]["id"] = tool_call_delta.id
                                if tool_call_delta.function:
                                    if tool_call_delta.function.name:
                                        collected_tool_calls[idx]["name"] = tool_call_delta.function.name
                                    if tool_call_delta.function.arguments:
                                        collected_tool_calls[idx]["arguments"] += tool_call_delta.function.arguments

                        if chunk.usage:
                            usage_from_api = True
                            total_prompt_tokens += chunk.usage.prompt_tokens or 0
                            total_completion_tokens += chunk.usage.completion_tokens or 0
                    break

                except Exception as e:
                    if retry_attempt < 9:
                        backoff = min(2**retry_attempt, 60)
                        logger.info(f"API error on attempt {retry_attempt + 1}/10: {e}. Retrying in {backoff}s...")
                        await asyncio.sleep(backoff)
                        current_text = ""
                        collected_tool_calls = {}
                        continue
                    else:
                        logger.error(f"API failed after 10 attempts: {e}")
                        raise

            if finish_reason == "length":
                print(f"WARNING: Hit max_tokens limit ({max_tokens}). Response was truncated.", flush=True)
                yield yield_event("error", content=f"Request exceeded max_tokens {max_tokens}")

            tool_calls = list(collected_tool_calls.values())

            api_message = {"role": "assistant"}
            if current_text:
                api_message["content"] = current_text
            else:
                api_message["content"] = None

            if tool_calls:
                api_message["tool_calls"] = [
                    {"id": tc["id"], "type": "function", "function": {"name": tc["name"], "arguments": tc["arguments"]}}
                    for tc in tool_calls
                ]

            messages.append(api_message)

            transcript_content = []
            if current_text:
                transcript_content.append({"type": "text", "text": current_text})
            for tc in tool_calls:
                transcript_content.append({
                    "type": "tool_use",
                    "id": tc["id"],
                    "name": tc["name"],
                    "input": json.loads(tc["arguments"]) if isinstance(tc["arguments"], str) else tc["arguments"],
                })

            transcript_message = {"role": "assistant", "content": transcript_content if transcript_content else []}
            if tool_calls:
                transcript_message["tool_calls"] = api_message["tool_calls"]
            yield yield_event("message", message=transcript_message)

            if not current_text and not tool_calls:
                break

            if tool_calls:
                tool_responses = []

                for tool_call in tool_calls:
                    try:
                        args = json.loads(tool_call["arguments"])
                        yield yield_event("tool_use_start", tool=tool_call["name"])
                        yield yield_event("tool_executing", tool=tool_call["name"], args=args)

                        result = await mcp_client.execute_tool(tool_call["name"], args)

                        result_text = ""
                        if result.content:
                            for c in result.content:
                                if hasattr(c, "text"):
                                    result_text += c.text

                        is_err = bool(getattr(result, "isError", False))
                        if is_err:
                            error_text = result_text or "Tool returned error with no message"
                            tool_responses.append({
                                "role": "tool",
                                "tool_call_id": tool_call["id"],
                                "content": f"Error: {error_text}",
                                "is_error": True,
                            })
                        else:
                            tool_responses.append({
                                "role": "tool",
                                "tool_call_id": tool_call["id"],
                                "content": result_text,
                                "is_error": False,
                            })

                        # Track if submit_report was called
                        if tool_call["name"] == "submit_report":
                            report_submitted = True

                    except Exception as e:
                        import traceback
                        error_msg = str(e) if str(e) else f"{type(e).__name__}: {repr(e)}"
                        print(f"Error executing tool {tool_call['name']}: {error_msg}")
                        yield yield_event("error", content=error_msg)
                        tool_responses.append({
                            "role": "tool",
                            "tool_call_id": tool_call["id"],
                            "content": f"Error: {error_msg}",
                            "is_error": True,
                        })

                for resp in tool_responses:
                    clean_resp = {
                        "role": resp["role"],
                        "tool_call_id": resp["tool_call_id"],
                        "content": resp["content"],
                    }
                    messages.append(clean_resp)

                tool_result_content = []
                for resp in tool_responses:
                    tool_result_content.append({
                        "type": "tool_result",
                        "is_error": resp.get("is_error", False),
                        "tool_use_id": resp["tool_call_id"],
                        "content": [{"type": "text", "text": resp["content"]}],
                    })

                transcript_tool_result_message = {"role": "user", "content": tool_result_content}
                yield yield_event("message", message=transcript_tool_result_message)

                continue
            else:
                if current_text and not report_submitted:
                    import re
                    report_match = re.search(
                        r'\{\s*"contract_address"\s*:\s*"[^"]*"\s*,\s*"risk_categories"\s*:\s*\[[^\]]*\]\s*\}',
                        current_text,
                    )
                    report_text = None
                    if report_match:
                        report_text = report_match.group(0)
                    else:
                        partial = re.search(
                            r'\{\s*"contract_address"\s*:\s*"[^"]*"\s*,\s*"risk_categories"\s*:\s*\[([^\]]*?)(?:\]|\s*$)\s*\}?',
                            current_text,
                        )
                        if partial:
                            addr_match = re.search(r'"contract_address"\s*:\s*"([^"]*)"', partial.group(0))
                            cats_match = re.search(r'"risk_categories"\s*:\s*\[([^\]]*?)(?:\]|\s*$)', partial.group(0))
                            if addr_match and cats_match:
                                raw_cats = cats_match.group(1).strip()
                                if raw_cats:
                                    cats = re.findall(r'"([^"]*)"', raw_cats)
                                    if cats:
                                        report_text = json.dumps({"contract_address": addr_match.group(1), "risk_categories": cats})

                    if report_text:
                        try:
                            rd = json.loads(report_text) if isinstance(report_text, str) else report_text
                            if isinstance(rd, str):
                                rd = json.loads(rd)
                            if "contract_address" in rd and "risk_categories" in rd:
                                print("Auto-submitting report detected in model text response")
                                yield yield_event("text", content="\n[Auto-submitting detected report...]\n")
                                result = await mcp_client.execute_tool("submit_report", {"report_json": json.dumps(rd)})
                                result_text = ""
                                if result.content:
                                    for c in result.content:
                                        if hasattr(c, "text"):
                                            result_text += c.text
                                report_submitted = True
                                messages.append({"role": "tool", "tool_call_id": "auto_submit", "content": result_text})
                                yield yield_event("message", message={"role": "user", "content": [{"type": "tool_result", "is_error": False, "tool_use_id": "auto_submit", "content": [{"type": "text", "text": result_text}]}]})
                                continue
                        except (json.JSONDecodeError, Exception):
                            pass

                    # If model wrote analysis but didn't submit, ask it to submit now
                    msg = 'Please submit your final report by calling the submit_report tool. Use this format: {"contract_address": "' + mcp_client.problem_id + '", "risk_categories": ["category1", "category2"]}. Only call the tool, no additional text.'
                    messages.append({"role": "user", "content": msg})
                    yield yield_event("message", message={"role": "user", "content": msg})
                    continue
                break

        # Grade
        grade_called = True

        try:
            transcript_str = json.dumps(messages, default=str)

            grade = await mcp_client.execute_tool(
                "grade_problem",
                {
                    "problem_id": mcp_client.problem_id,
                    "transcript": transcript_str,
                },
            )

            if grade.isError:
                error_text = "".join(c.text for c in grade.content if hasattr(c, "text"))
                yield yield_event("error", content=error_text)
                raise RuntimeError(error_text)

            assert grade.content is not None
            assert isinstance(grade.content, list)
            assert len(grade.content) >= 1

            grade_text = grade.content[0].text if hasattr(grade.content[0], "text") else ""
            if not grade_text.strip():
                grade_data = {"score": 0, "error": "Empty grade response"}
            else:
                grade_data = json.loads(grade_text)

            yield yield_event("grade", grade=grade_data)
            if not usage_from_api:
                total_prompt_tokens = sum(len(str(m)) for m in messages) // 4
                total_completion_tokens = sum(
                    len(str(m.get("content", ""))) for m in messages if m.get("role") == "assistant"
                ) // 4
            yield yield_event("token_usage",
                prompt_tokens=total_prompt_tokens,
                completion_tokens=total_completion_tokens,
            )
            yield yield_event("done")

        except Exception as e:
            print(f"Grading error: {e}")
            yield yield_event("error", content=str(e))
            if not usage_from_api:
                total_prompt_tokens = sum(len(str(m)) for m in messages) // 4
                total_completion_tokens = sum(
                    len(str(m.get("content", ""))) for m in messages if m.get("role") == "assistant"
                ) // 4
            yield yield_event("token_usage",
                prompt_tokens=total_prompt_tokens,
                completion_tokens=total_completion_tokens,
            )
            yield yield_event("done")

    except Exception as e:
        print(f"Error during stream: {e}")
        yield yield_event("error", content=str(e))
        if not usage_from_api:
            total_prompt_tokens = sum(len(str(m)) for m in messages) // 4
            total_completion_tokens = sum(
                len(str(m.get("content", ""))) for m in messages if m.get("role") == "assistant"
            ) // 4
        yield yield_event("token_usage",
            prompt_tokens=total_prompt_tokens,
            completion_tokens=total_completion_tokens,
        )
        yield yield_event("done")
    finally:
        if not grade_called:
            try:
                print("Attempting to grade despite error...")
                transcript_str = json.dumps(messages, default=str)
                grade = await mcp_client.execute_tool(
                    "grade_problem",
                    {"problem_id": mcp_client.problem_id, "transcript": transcript_str},
                )
                if not grade.isError and grade.content:
                    grade_text = grade.content[0].text if hasattr(grade.content[0], "text") else ""
                    if grade_text.strip():
                        yield yield_event("grade", grade=json.loads(grade_text))
                if not usage_from_api:
                    total_prompt_tokens = sum(len(str(m)) for m in messages) // 4
                    total_completion_tokens = sum(
                        len(str(m.get("content", ""))) for m in messages if m.get("role") == "assistant"
                    ) // 4
                yield yield_event("token_usage",
                    prompt_tokens=total_prompt_tokens,
                    completion_tokens=total_completion_tokens,
                )
                yield yield_event("done")
            except Exception as grade_error:
                print(f"Failed to grade after error: {grade_error}")

        await mcp_client.cleanup()
